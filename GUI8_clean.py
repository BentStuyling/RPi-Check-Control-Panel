...
class MainApp:
    def __init__(self):
        ...
        # Fonts
        global TitleFont
        TitleFont = Font(family="Helvetica", size=18)
        global LabelFont
        ...
        self.root.bind('<F10>', self.smallwindow)
        ...
        frame = PageOne(parent=container, controller=self.root)
        frame.main.config(bg="black")
        self.frames[page_name] = frame
        # Put al frames in same loation
        frame.main.grid(row=0, column=0, sticky="nsew")
    ...
    def smallwindow(self, event):
        self.root.attributes('-fullscreen', False)
        self.root.geometry('{}x{}'.format(240, 320))
...
class PageOne:
    def __init__(self, parent, controller):
        ...
        # Create frames for labels and values to display
        tk.Label(self.main, font=TitleFont, fg=Labelcolor, bg='black', text='MOTOR').grid(row=0)
        # Line canvas
        canv1 = tk.Canvas(self.main, width=240, height=2, bg='black', highlightthickness=0)
        canv1.grid(row=1)
        canv1.create_line(0, 1, 240, 1, fill='white', width=2)

        f_data = tk.Frame(self.main)
        f_data.grid(row=2)
    ...
    #     frame_lable_row_1 = tk.Frame(self.main, width=240, height=30, bg='black')
    #     frame_lable_row_1.grid(row=2)
    #     frame_lable_row_1.grid_propagate(0)
    #     frame_value_row_1 = tk.Frame(self.main, width=240, height=35, bg='black')
    #     frame_value_row_1.grid(row=3)
    #     frame_value_row_1.grid_propagate(0)
    #     # Secon rowd frames for lables and values
    #     frame_lable_row_2 = tk.Frame(self.main, width=240, height=30, bg='black')
    #     frame_lable_row_2.grid(row=4)
    #     frame_lable_row_2.grid_propagate(0)
    #     frame_value_row_2 = tk.Frame(self.main, width=240, height=35, bg='black')
    #     frame_value_row_2.grid(row=5)
    #     frame_value_row_2.grid_propagate(0)
    #     # Line canvas
    #     linecanvas_2 = tk.Canvas(self.main, width=240, height=10, bg='black', highlightthickness=0)
    #     linecanvas_2.grid(row=6)
    #     linecanvas_2.grid_propagate(0)
    #     # Plot frame
    #     Plotframe = tk.Frame(self.main, width=240, height=148, bg='black')
    ...
    def _to_GUI(self):
        ...
        for pos, font, color, text in zip(positions, fonts, colors, texts):
            tk.Label(
                ...
                text=text,
                textvariable=text
            ).grid(row=pos['row'], column=pos['col'], columnspan=pos['span'], sticky=pos['sticky'])
            # ).place(
            #     x=pos['x'],
            #     y=pos['y'],
            #     anchor=pos['anchor']
